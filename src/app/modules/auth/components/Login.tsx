...
interface ToolData {
  toolId: number;
  toolName: string;
  toolOrgStructure: ToolOrgStructure[];
}
...
interface AuthData {
  ...
  roleName: string;
  orgId: number;
  globalAdminRole: number;
  clientAdminRole: number;
  openTaskCount: number;
  defaultPassword: boolean;
  orgToolsData: ToolData[];
  isSuccess: boolean;
  message: string | null;
...
export function Login() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setStatus, setSubmitting }) => {
      setLoading(true);
      try {
        const authData: AuthData = await fetchAuthenticate(values.username, values.password, values.org);
        ...
        if (authData.isSuccess) {
          ...
          if (authData.orgToolsData && authData.orgToolsData.length > 0) {
            ...
            toolData.toolOrgStructure.forEach((level: ToolOrgStructure) => {
