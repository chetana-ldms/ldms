...
const DetailsModal = ({ show, onClose, incidentData, onSave }) => {
  ...
  /* ---------------------------------------------------
      LOAD EXISTING HTML + BACKEND ATTACHMENTS
  ---------------------------------------------------*/
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Convert backend base64 → File object (binary)
        ...
        // Replace cid inline with base64 preview
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res?.isSuccess) {
        onSave({
          IncidentDetails: cleanedHtml,
          IncidentDetails_Attachments: finalFiles,
        });
      }
