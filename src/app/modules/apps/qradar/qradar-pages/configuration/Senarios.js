...
const Senarios = () => {
  ...
  const filteredList = (filterValue
    ? tools.filter((item) => (item.scenarioName || '').toLowerCase().includes(filterValue.toLowerCase()))
    : [...(tools || [])]
  ).sort((a, b) => {
  ...
  const currentItems = filteredList ? filteredList.slice(indexOfFirstItem, indexOfLastItem) : null
  ...
  const exportToExcel = async () => {
    ...
    csvContent +=
      ...
      dataToExport
        .map(
          (item) =>
            `${item.scenarioName || ''},${item.parentScenarioName || ''}`
        )
        .join('\n')
      ...
      <div className='card-header no-pad'>
        <h3 className='card-title align-items-start flex-column'>
          <span className='card-label fw-bold fs-3 mb-1'>
            Senarios ({currentItems ? currentItems.length : 0} /{' '}
          ...
          <tbody>
            ...
            {currentItems !== null ? (
              currentItems.map((item, index) => (
                <tr key={item.scenarioId} className='fs-12 table-row'>
                  <td>{item.scenarioName}</td>
                  <td>{item.parentScenarioName}</td>
                </tr>
              ))
            ) : (
              <tr>
