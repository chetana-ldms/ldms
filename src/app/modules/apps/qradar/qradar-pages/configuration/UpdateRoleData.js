...
const UpdateRoleData = () => {
  ...
  const reload = async () => {
    try {
      setLoading(true)
      const data = await fetchRolesUrl(role.orgId, modifiedUserId)
      setRoles(data)
      setLoading(false)
    } catch (error) {
      console.log(error)
  ...
  const handleSubmit = async (event) => {
    ...
    if (!role.orgId) {
      notifyFail('select a Organization')
      setLoading(false)
      return
    }
          ...
          <div className='row mb-6 table-filter'>
            <div className='col-lg-4 mb-4 mb-lg-0'>
              <div className='fv-row mb-0'>
                <label className='form-label fs-6 fw-bolder mb-3'>Organization</label>
                <select
                  className='form-select form-select-lg form-select-solid'
                  value={role.orgId || ''}
                  onChange={(e) => setRole({...role, orgId: Number(e.target.value)})}
                  disabled={!!save}
                >
                  <option value=''>Select Organization</option>
                  {Number(sessionStorage.getItem('globalAdminRole')) === 1 &&
            ...
            <div className='col-lg-4 mb-4 mb-lg-0'>
              <div className='fv-row mb-0'>
                ...
                <select
                  ...
                  onChange={(e) => setRole({...role, parentRoleId: Number(e.target.value)})}
                >
