...
const TemplateGroupes = () => {
  ...
  const confirmDelete = async () => {
    if (itemToDelete) {
      ...
      try {
        const response = await fetchMessageTemplateGroupDeleteUrl(data)
  ...
  const reload = async () => {
    try {
      setLoading(true)
      const response = await fetchMessageTemplateGroupsUrl(0)
  ...
  const handleNavigateToUpdate = (id) => {
    navigate(`/qradar/template-groupes/update/${id}`, {state: {save: true}})
  }
  return (
      ...
      <div className='card-header no-pad'>
        <h3 className='card-title align-items-start flex-column'>
          <span className='card-label fw-bold fs-3 mb-1'>
            Template Groupes ({currentItems ? currentItems.length : 0} /{' '}
        ...
        <div className='card-toolbar'>
          <div className='d-flex align-items-center gap-2 gap-lg-3'>
            <Link
              to='/qradar/template-groupes/add'
                    ...
                    {isActionAuthorized('Update') ? (
                      <span>
                        <Link
                          ...
                          to={`/qradar/template-groupes/update/${item.masterId}`}
                          title='Edit'
                        >
