...
function SlaMeasurement() {
  ...
  const filteredData365 = alertData?.filter((alert) => alert.summeryPeriod === '365-Days')
  const filteredData7 = alertData?.filter((alert) => alert.summeryPeriod === '7-Days')
  const filteredData30 = alertData?.filter((alert) => alert.summeryPeriod === '30-Days')
  ...
  if (filteredData365) {
    ...
    alertCount365 = filteredData365.map((alert) => alert.summeryPeriodAlertCount)
  }
  ...
  return (
    <div>
      {loading ? (
        ...
        <>
          <h2 className='mb-10'>SLA measurement for the last year (2/20/2020 and 7/28/2021)</h2>
          <Tabs>
