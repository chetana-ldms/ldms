...
const ChannelsPage = () => {
  ...
  const renderChannelTemplate = (channel) => {
    switch (channel.channelTypeName) {
      case 'Report':
        return <Reports channelId={channel.channelId} channelName={channel.channelName} />
      case 'Document':
        return <Document channelId={channel.channelId} channelName={channel.channelName} />
      case 'QuestionAndAnswer':
        return <QA channelId={channel.channelId} channelName={channel.channelName} />
      case 'UnderConstruction':
        return <UnderConstruction channelId={channel.channelId} channelName={channel.channelName} />
      case 'UnderReview':
        return <UnderReview channelId={channel.channelId} channelName={channel.channelName} />
      default:
        return (
          <div>
  ...
  return (
    <div className='channel-list channels-page'>
      ...
      <div className='channel-title'>
        ...
        {isActionAuthorized('Create') ? (
          <span className='float-right add-btn btn-new' onClick={() => setShowModal(true)}>
            Add Channel
          </span>
        ) : (
          <span
          ...
          <Tabs className='vertical-tabs'>
            ...
            {channels.map((channel) => (
              <TabPanel key={channel.channelId} className='channel-chat'>
                <div className='tab-content pt-5'>{renderChannelTemplate(channel)}</div>
              </TabPanel>
            ))}
          </Tabs>
        ) : (
