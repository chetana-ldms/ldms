...
const CreateExclusionModalEdit = ({
  show,
  onClose,
  refreshParent,
  selectedItem,
  ...
  const initialExcludeAlertsFile =
    selectedItem?.actions?.includes("upload") || false;
  const initialexcludeBinaryVaultFile =
    selectedItem?.actions?.includes("detect") || false;
  ...
  const [selectedValue, setSelectedValue] = useState(
    mapTypeToSelectedValue(selectedItem?.type)
  );
  ...
  const [excludeAlertsFile, setExcludeAlertsFile] = useState(
    initialExcludeAlertsFile
  );
